...
CHECKS_MIN=${CHECKS_MIN:-16}   # 18 on a box with no blender on PATH; the skip case cannot run everywhere
...
chmod +x "$TMP/venv/bin/articraft" "$TMP/venv/bin/python" "$FAKE/render/render.sh" "$TMP/venv/bin/curl"
...
for code in "0   the object was built" "2   this script could not start" "3   the frozen dresser inputs" "4   the object scored PASS"; do
